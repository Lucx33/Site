...
fetch("http://localhost:3000/restaurantes",).then((response) => response.json()).then((data) => {
    document.getElementById("nome").innerHTML += (data[2].nome)
    document.getElementById("horarios").innerHTML += (data[2].horario)
    document.getElementById("loc").innerHTML += (data[2].localizacao)
    document.getElementById("comida").innerHTML += (data[2].tipoEspec)
...
function Avaliar(estrela) {
    ...
    url = url.split("BarraquinhaStrogonoff.html");
    url = url[0];
   ...
   const api_url = "http://localhost:3000/BarraquinhaStrogonoff"
   ...
   botao.addEventListener('click', () => {
       const data = document.getElementById("comentario").value
        //Trocar o caminho para o restaurante
       fetch("http://localhost:3000/BarrarquinhaStrogonoff", {
           method: 'POST', // or 'PUT'
