...
fetch("http://localhost:3000/restaurantes",).then((response) => response.json()).then((data) => {
    document.getElementById("nome").innerHTML += (data[4].nome)
    document.getElementById("horarios").innerHTML += (data[4].horario)
    document.getElementById("loc").innerHTML += (data[4].localizacao)
    document.getElementById("comida").innerHTML += (data[4].tipoEspec)
...
function Avaliar(estrela) {
   ...
   botao.addEventListener('click', () => {
       const data = document.getElementById("comentario").value
        //Trocar o caminho para o restaurante
       fetch("http://localhost:3000/BarraquinhaWrap", {
           method: 'POST', // or 'PUT'
