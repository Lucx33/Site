...
fetch("http://localhost:3000/restaurantes",).then((response) => response.json()).then((data) => {
    document.getElementById("nome").innerHTML += (data[14].nome)
    document.getElementById("horarios").innerHTML += (data[14].horario)
    document.getElementById("loc").innerHTML += (data[14].localizacao)
    document.getElementById("comida").innerHTML += (data[14].tipoEspec)
...
function Avaliar(estrela) {
   ...
   botao.addEventListener('click', () => {
       const data = document.getElementById("comentario").value
        //Trocar o caminho para o restaurante
       fetch("http://localhost:3000/CouveFlor", {
           method: 'POST', // or 'PUT'
