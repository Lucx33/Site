...
fetch("http://localhost:3000/restaurantes",).then((response) => response.json()).then((data) => {
    document.getElementById("nome").innerHTML += (data[11].nome)
    document.getElementById("horarios").innerHTML += (data[11].horario)
    document.getElementById("loc").innerHTML += (data[11].localizacao)
    document.getElementById("comida").innerHTML += (data[11].tipoEspec)
...
function Avaliar(estrela) {
   ...
   botao.addEventListener('click', () => {
       const data = document.getElementById("comentario").value
        //Trocar o caminho para o restaurante
       fetch("http://localhost:3000/Geneal", {
           method: 'POST', // or 'PUT'
