...
fetch("http://localhost:3000/restaurantes",).then((response) => response.json()).then((data) => {
    document.getElementById("nome").innerHTML += (data[6].nome)
    document.getElementById("horarios").innerHTML += (data[6].horario)
    document.getElementById("loc").innerHTML += (data[6].localizacao)
    document.getElementById("comida").innerHTML += (data[6].tipoEspec)
...
function Avaliar(estrela) {
   ...
   botao.addEventListener('click', () => {
       const data = document.getElementById("comentario").value
        //Trocar o caminho para o restaurante
       fetch("http://localhost:3000/NeiMate", {
           method: 'POST', // or 'PUT'
