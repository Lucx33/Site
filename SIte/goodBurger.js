...
fetch("http://localhost:3000/restaurantes",).then((response) => response.json()).then((data) => {
    document.getElementById("nome").innerHTML += (data[0].nome)
    document.getElementById("horarios").innerHTML += (data[0].horario)
    document.getElementById("loc").innerHTML += (data[0].localizacao)
    document.getElementById("comida").innerHTML += (data[0].tipoEspec)
...
function Avaliar(estrela) {
   ...
   const api_url = "http://localhost:3000/GoodBurger"
   ...
   botao.addEventListener('click', () => {
       const data = document.getElementById("comentario").value
        //Trocar o caminho para o restaurante
       fetch("http://localhost:3000/GoodBurger", {
           method: 'POST', // or 'PUT'
